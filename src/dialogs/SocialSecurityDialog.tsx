...
import { eventTypeIcons, isAutoGeneratedSSName, generateSSDefaultName } from '../utils/defaultName';
...
const makeDefaultFormData = () => ({
  ...
  ssHaircutPercent: DEFAULT_SS_HAIRCUT_PERCENT,
  ssHaircutYear: DEFAULT_SS_HAIRCUT_YEAR,
});
...
const SocialSecurityDialog: React.FC<SocialSecurityDialogProps> = ({
  ...
  useEffect(() => {
    ...
    if (editEvent) {
      ...
      setFormData({
        ...
        ssHaircutPercent: editEvent.ssHaircutPercent ?? DEFAULT_SS_HAIRCUT_PERCENT,
        ssHaircutYear: editEvent.ssHaircutYear ?? DEFAULT_SS_HAIRCUT_YEAR,
      });
    } else {
  ...
  }, [visible, editEvent, isMfj, selfTaken, spouseTaken, existingSSEvents, currentAge, spouseAge]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      ssHaircutYear: formData.ssHaircutYear,
            ...
            {formData.ssHaircutEnabled && (
              <>
                <InputGroup>
                  ...
                  <InputNumber
                    value={formData.ssHaircutYear}
                    onValueChange={(e) =>
                      setFormData({ ...formData, ssHaircutYear: e.value ?? DEFAULT_SS_HAIRCUT_YEAR })
                    }
                    useGrouping={false}
                    min={2025}
                    max={2100}
                ...
                <InputGroup>
                  ...
                  <InputNumber
                    ...
                    onValueChange={(e) =>
                      setFormData({ ...formData, ssHaircutPercent: e.value ?? DEFAULT_SS_HAIRCUT_PERCENT })
                    }
                    suffix='%'
                    min={0}
                    max={100}
                  />
                  <HelpText>
                    Latest trustees estimate (2026 report): ~{DEFAULT_SS_HAIRCUT_PERCENT}% reduction
                    when the trust fund is exhausted around {DEFAULT_SS_HAIRCUT_YEAR}. Edit either
                    to model your own assumption.
                  </HelpText>
                </InputGroup>
              </>
            )}
