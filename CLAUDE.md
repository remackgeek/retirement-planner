...
- `src/utils/defaultName.ts` — `eventTypeLabels`, `goalTypeLabels`, default name generators,
  SS name helpers (`isAutoGeneratedSSName`, `generateSSDefaultName`)
...
Run `npm run lint`, `npm run test`, and `npm run build` to verify changes — all three are
CI gates (lint must be zero errors AND zero warnings; deploy.yml runs them before publish).
...
`AppHeader` renders four dropdown menus (PrimeReact `Menu` popups):
...
- **Tools**
  ...
  - **Social Security** — the claiming-age wizard (`SocialSecurityWizardDialog`).
  - **Roth Conversions** — the multi-year conversion planner (`RothConversionDialog` with `onApplyBatch`).
- **Settings**
...
Items that operate on the active scenario (Modeling, Cash Bucket, Tax & IRS, Export CSV, the wizards) are disabled when there is no active scenario; Load example… is always enabled.
